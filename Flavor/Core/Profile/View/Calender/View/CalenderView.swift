            ...
            ScrollView(.vertical, showsIndicators: false){
                ...
                //ALL DATES
                        ...
                        if hasSelectedDate{
                            ...
                            ForEach(Array(profileViewModel.calenderStorys.filter{ isSameDay(date1: $0.timestamp.dateValue(), date2: currentDate) }.filter{!homeVM.deletedStorys.contains($0.id)}.sorted(by: {$0.timestamp.dateValue() > $1.timestamp.dateValue()}).enumerated()), id: \.element.id) { index, story in
